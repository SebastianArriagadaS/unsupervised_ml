...
results["label_1st"] = results["label_1st"].str.replace("_", " ")
results["label_2nd"] = results["label_2nd"].str.replace("_", " ")
results["label_3rd"] = results["label_3rd"].str.replace("_", " ")

# count how many times the correct label appears in the first, second and third position
results["1st"] = results["correct_label"] == results["label_1st"]
results["2nd"] = results["correct_label"] == results["label_2nd"]
results["3rd"] = results["correct_label"] == results["label_3rd"]
...
for label in labels:
    ...
    # count how many times the correct label appears in the first, second and third position
    df["1st"] = df["correct_label"] == df["label_1st"]
    df["2nd"] = df["correct_label"] == df["label_2nd"]
    df["3rd"] = df["correct_label"] == df["label_3rd"]
    # sum the results
    acc_1st = df["1st"].sum()
    acc_2nd = df["2nd"].sum()
    acc_3rd = df["3rd"].sum()
